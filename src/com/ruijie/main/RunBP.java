...
public class RunBP {
	...
	private static Logger logger = LogManager.getLogger(RunBP.class);
		...
		if(preditBuilding == -1){
			...
			if(predictStore==0){
				sqlLocation = "select distinct building_id,floor_id,store_id from t_common_stat_for_predict " +
						" where building_id>0 and hour=24 and store_id=-1 and floor_id=-1";
			}else{
				sqlLocation = "select distinct building_id,floor_id,store_id from t_common_stat_for_predict " +
						" where building_id>0 and hour=24 and floor_id=-1";
			}
			...
			if(predictStore==0){
				sqlLocation = "select distinct building_id,floor_id,store_id from t_common_stat_for_predict " +
						" where building_id=" + preditBuilding +" and hour=24 and store_id=-1 and floor_id=-1";
			}else{
				sqlLocation = "select distinct building_id,floor_id,store_id from t_common_stat_for_predict " +
						" where building_id=" + preditBuilding + " and hour=24 and floor_id=-1";
			}
		}
...
/*		//遍历各个building, floor, store进行预测
		...
		for(int[] location:locationList){
			...
			mapDate2allFeature = buildingDate2allFeature.get(buildingID);
			trainAndPredict(FactorQuantify.OLDNUM, buildingID, floorID,
					storeID, false,mapDate2allFeature, fromDateStr, numDays);

			logger.info("location:" + buildingID + "|" + floorID + "|" + storeID + ",预测类别：天,预测指标:stay_rate");
			mapDate2allFeature = buildingDate2allFeature.get(buildingID);
			trainAndPredict(FactorQuantify.STAYRATE, buildingID, floorID,
					storeID, false, mapDate2allFeature, fromDateStr, numDays);

			logger.info("location:" + buildingID + "|" + floorID + "|" + storeID + ",预测类别：天,预测指标:duration");
			mapDate2allFeature = buildingDate2allFeature.get(buildingID);
			trainAndPredict(FactorQuantify.DURATION, buildingID, floorID,
						storeID, false,mapDate2allFeature, fromDateStr, numDays);

			logger.info("location:" + buildingID + "|" + floorID + "|" + storeID + ",预测类别：天,预测指标:enter_rate");
			mapDate2allFeature = buildingDate2allFeature.get(buildingID);
			trainAndPredict(FactorQuantify.ENTERRATE, buildingID, floorID,
						storeID, false,mapDate2allFeature, fromDateStr, numDays);
...
/*		for(Map.Entry<String, float[]> m: date2allFeatures.entrySet()){
		...
		traindata.normalize();//归一化数据(input的数据)
		...
		try{
			//创建神经网络集成
			//traindata.getNoOfInputs:输入数据的维度个数
			//para.getNoOfHidNodes:隐层节点数
			//traindata.getNoOfClasses:预测真实值的维度个数,有多少个类
